        ...
        database.ref('Contests').once('value').then(function (snapshot) {
            for (var i = 1; i <= 3; i++) {
                ...
                if (! finished) {
                    ...
                    card10.innerHTML = "<h3 class='card-title'>" + category + "</h3> <h5>THIS CONTEST FINISHED</h5> <p>The new cycle begins in:</p> <h3 id='countdown" + i + "' class='dashboard-numbers2' style='text-align: center;'>...</h3> <button type='button' class='btn btn-new' id='stats1'>SEE STATS</button>";
                    card10.style.visibility = "visible";
                    setTime(i);
                    $(".loader2").fadeOut("slow");
